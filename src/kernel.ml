...
let is_arrow = function Arr _ -> true | _ -> false
...
let rec mem t = function [] -> false | x :: xs -> eq_term t x || mem t xs
...
let verif_axiom t l =
  if mem t l then ()
    ...
    failwith
      "could not apply the axiom: the conclusion of the sequent does not \
...
let verif_abstraction seq1 seq2 =
  ...
  if is_arrow t1 then
    ...
    if eq_term t2 b then
      ...
      failwith
        "the conclusion of the sequent does not correspond to the right of the \
         arrow"
  else
    failwith
      "the abstraction rule was applied on a term which does not contain an \
       arrow"
...
let verif_modus_ponens seq1 seq2 seq3 =
  ...
  if is_arrow t2 then
    ...
    if eq_term t1 b then
      if eq_term t3 a then
        if eq_ctx c1 c2 && eq_ctx c2 c3 then ()
        else
          failwith
            "failure while applying modus ponens: the contexts are different"
      else
        failwith
          "the premise does not match to the antecedent of the implication"
      ...
      if eq_term t1 b then
        ...
        failwith
          "the conclusion of the sequent does not match to the consequent of \
           the implication"
    else
      failwith
        "cannot apply the modus ponens because no premises has an arrow as \
         conclusion"
  else if is_arrow t3 then
    let a, b = split_arrow t3 in
    if eq_term t1 b then
      ...
      failwith
        "the conclusion of the sequent does not match to the consequent of the \
         implication"
  else
    failwith
      "cannot apply the modus ponens because no premises has an arrow as \
...
let verif_and_intro seq1 seq2 seq3 =
  ...
  if eq_ctx c1 c2 && eq_ctx c2 c3 then
    ...
    failwith
      "failure while applying the introduction of and: the contexts are \
       different"
  ...
  if is_or t1 then
    ...
    if eq_term a t2 then
      if eq_ctx c1 c2 then ()
        ...
        failwith
          "the contexts are not equal: failure during the application of \
           or-intro-left rule"
    else
      failwith
        "the formulae does not match: you may try to apply or-intro-r instead \
         or simply the rule is not correct"
...
let verif_or_intror seq1 seq2 =
  ...
  if is_or t1 then
    ...
    if eq_term a t2 then
      ...
      failwith
        "the formulae does not match: you may try to apply or-intro-l instead \
         or simply the rule is not correct"
  ...
  if is_or t2 then
    ...
    if mem a c3 && mem b c4 then
      ...
      if eq_ctx c1 c2 && eq_ctx c2 c3' && eq_ctx c3' c4' then
        if eq_term t1 t3 && eq_term t3 t4 then true else false
      else false
    else false
  else false
...
let verif_top_intro seq1 =
  ...
  if is_top t1 then ()
    ...
    failwith "the top rule was applied on a sequent whose conclusion is not top"
...
let verif_bottom_elim seq1 seq2 =
  ...
  if eq_term t2 Bottom then
    ...
    failwith
      "bottom is not the conclusion of the premise sequent in the use of the \
       bottom-elim rule"
...
let rec verif_proof_term p =
  ...
  | Empty (s, r) -> (
      ...
      | _ ->
          failwith
            "the leaves of the proof term could be only labelled by an axiom \
             rule or a top-intro rule")
  | Unary (s, r, p') -> (
      match r with
      | Abstraction ->
          let s' = current_sequent p' in
          verif_abstraction s s';
          verif_proof_term p'
      | OrIntrol ->
          let s' = current_sequent p' in
          verif_or_introl s s';
  ...
  | Binary (s, r, p1, p2) -> (
      ...
      match r with
      | ModusPonens -> verif_modus_ponens s s1 s2
      | AndIntro -> verif_and_intro s s1 s2
      | AndElim -> verif_and_elim s s1 s2
      | _ ->
          failwith
            "the rule used is not binary but it is a label of an unary node in \
             the proof tree")
